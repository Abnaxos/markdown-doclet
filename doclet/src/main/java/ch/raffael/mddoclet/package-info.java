 ...
 * recognised as such by Markdown. To work around this problem, the Doclet uses a simple
 ...
 * ------------
 *
 * The following known tags handled by Markdown so you can use Markup with them:
 *
 *  *  `@author`
 *  *  `@version`
 *  *  `@return`
 ...
 * [issue #7](https://github.com/Abnaxos/markdown-doclet/issues/7)). It's recommended not
 * to use them for now.
 *
 * ### Custom Tag Handling
 *
 * Tag handling can be customised by implementing your own `TagRenderer`s and registering
 * them with the MarkdownDoclet. You'll have to write your own Doclet, though, there's
